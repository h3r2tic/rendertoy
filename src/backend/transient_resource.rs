...
pub trait TransientResource: Clone {
    ...
    fn allocate_payload(gl: &gl::Gl, desc: Self::Desc) -> Self::Allocation;
}
...
pub fn create_transient<Res: TransientResource>(gl: &gl::Gl, desc: Res::Desc) -> Res {
    ...
    let alloc = if existing.is_empty() {
        ...
        TransientResourceAllocation {
            ...
            payload: Res::allocate_payload(gl, desc),
        }
    } else {
